...
- **Reestructura fuga/rescate en 2 tabs + FECHAS REALES de etapa (2026-07-24, feedback reunión de equipo):** los tabs Churn & Rescate / Fidelización+CS / Recuperación se fusionaron en DOS según diseño de Pauline: **"🔁 Gestión Comercial Recuperación"** (KPIs de cartera + Nuevos Perdidos del Mes + Transición a Fidelización — lo gestiona ventas, SIN filtro CS) y **"👩‍💼 Gestión ADMIN Fidelización"** (KPIs CS + Clientes Ganados + Clientes Recuperados + Nuevos Durmientes + Estacionales; Recuperados/Ganados con filtro `CS_GESTION`, Durmientes sin filtro porque es la cola de trabajo de admin). **Fix de fondo en `extract_crm.py`:** las tablas "nuevos durmientes/perdidos de la semana" mostraban SIEMPRE lo mismo porque el cron de Odoo re-estampa `write_date` en batch — ahora `last_update` de dormant/lost/rescued lists es la fecha REAL del cambio de etapa vía `mail.tracking.value` (cobertura 861/896 leads; fallback write_date). Con fechas reales los rescates del mes pasaron de 5 a 20 durmientes (42.170 L). ELIMINADO en la fusión: tabla "Clientes En Riesgo Crediticio" (el riesgo vive en CEO dashboard), "Todos los Durmientes", tabla "Recuperables ahora" del viejo tab Recuperación (solo se conservó Estacionales) y `renderRecovery()`. La "Tasa Rescate" del KPI usa `rescued_dormant+rescued_lost` (antes mostraba "0 rescatados" por campo inexistente).
