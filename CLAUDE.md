...
- **Tab Operaciones (2026-07-30):** nuevo tab "🚚 Operaciones" en `crm-sales.html`: SLA de entrega del MES ACTUAL (mismo método `extract_sla_data`: shipping_date vs 1ª factura por `invoice_origin`) + litros entregados por camión por día (30 días). Camión = `stock.warehouse` de la sale.order (HHPT-71, PHXC-44, PYHK-28, SHGP-60, TJVS-53, TYDG-23, VDKT-95); litros desde facturas posted → join `invoice_origin`. Nuevo bloque `operaciones` en `extract_crm.py` (`extract_operaciones`). ⚠ El SLA sale ~100% por construcción y Pauline lo sabe (auditado 30-jul): el 85% de las órdenes se crea con `shipping_date` a ≤1 día (la orden se digita cuando el despacho ya está resuelto), `shipping_date` NO tiene tracking (re-agendas invisibles) y las órdenes sin factura quedan fuera del cálculo. Decisión de Pauline: publicarlo así igual. Si se quiere un SLA con dientes: lead time create_date→factura (inmanipulable) o activar tracking en shipping_date. Los litros por camión SÍ son confiables (facturas reales).
