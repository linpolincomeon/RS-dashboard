...
- **Tab Operaciones (2026-07-30):** nuevo tab "🚚 Operaciones" en `crm-sales.html`: SLA de entrega del MES ACTUAL (mismo método `extract_sla_data`: shipping_date vs 1ª factura por `invoice_origin`) + litros entregados por camión por día (30 días). Camión = `stock.warehouse` de la sale.order (HHPT-71, PHXC-44, PYHK-28, SHGP-60, TJVS-53, TYDG-23, VDKT-95); litros desde facturas posted → join `invoice_origin`. Nuevo bloque `operaciones` en `extract_crm.py` (`extract_operaciones`). ⚠ El SLA por `shipping_date` sale ~100% por construcción (auditado 30-jul: 85% de órdenes con promesa a ≤1 día, campo sin tracking, órdenes sin factura excluidas) — quedó como KPI secundario "SLA Plan Despacho". **La métrica principal es "⏱ Entrega ≤48h": lead time `create_date`→1ª factura (fechas INMUTABLES), calculada por semana ENAP** (`sla_semanas[].lead_*` en el JSON), con tabla "🐢 Entregas Lentas (3+ días)". Julio real: 80–89% según semana. Todo el tab rinde por semana comercial (selector compartido) — la gestión del supervisor de operaciones se evalúa semanalmente. Los litros por camión vienen de facturas reales (confiables).
