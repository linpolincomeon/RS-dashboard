...
- ⚠ **Seguridad:** la `ODOO_KEY` quedó expuesta hardcodeada en el historial de git de este repo PÚBLICO (commit `90de552`, extract_ceo.py/extract_odoo.py). Pendiente: rotar la API key en Odoo (Preferencias → Seguridad de la cuenta) y actualizar el secret `ODOO_KEY` en GitHub Actions.
