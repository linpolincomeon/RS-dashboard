...
def extract_sales_data(models, uid, custom_start=None, custom_end=None, label_override=None):
    ...
    # margin_zone is a float fraction (e.g. 0.088 = 8.8%). Guard against >1 values.
    inv_margin_map = {}
    for i in invoices:
        m = i.get("margin_zone", 0) or 0
        if m > 1:
            m = m / 100  # was stored as percentage, normalize
        inv_margin_map[i["id"]] = m
    ...
    nc_margin_map = {}
    for n in ncs:
        m = n.get("margin_zone", 0) or 0
        if m > 1:
            m = m / 100
        nc_margin_map[n["id"]] = m
...
def extract_churn_data(models, uid):
    ...
    # Deduplicate by partner_id — keep the most recent lead per partner
    dormant_seen = {}  # pid -> {entry}
    rescued_seen = {}
    ...
    for lead in durmiente_leads:
        ...
        if pid and pid in curr_month_partners:
            if pid not in rescued_seen or write_date > rescued_seen[pid]["last_update"]:
                rescued_seen[pid] = {"name": name, "user": user, "last_update": write_date}
        else:
            if pid not in dormant_seen or write_date > dormant_seen[pid]["last_update"]:
                dormant_seen[pid] = {"name": name, "user": user, "last_update": write_date, "partner_id": pid}

    for entry in dormant_seen.values():
        dormant_list.append(entry)
        dormant_by_user[entry["user"]] += 1
    for entry in rescued_seen.values():
        rescued_dormant_list.append(entry)
        rescued_dormant_by_user[entry["user"]] += 1
    ...
    for lead in perdido_leads:
        ...
        if write_date >= fmt(month_start) and pid:
            newly_lost_pids.add(pid)

        if pid and pid in curr_month_partners:
            if pid not in rescued_lost_seen or write_date > rescued_lost_seen[pid]["last_update"]:
                rescued_lost_seen[pid] = {"name": name, "user": user, "last_update": write_date}
        else:
            if pid not in lost_seen or write_date > lost_seen[pid]["last_update"]:
                lost_seen[pid] = {"name": name, "user": user, "last_update": write_date, "partner_id": pid}

    newly_lost = len(newly_lost_pids)
    for entry in lost_seen.values():
        lost_list.append(entry)
        lost_by_user[entry["user"]] += 1
    for entry in rescued_lost_seen.values():
        rescued_lost_list.append(entry)
        rescued_lost_by_user[entry["user"]] += 1
