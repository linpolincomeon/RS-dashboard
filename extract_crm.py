...
CHURN_JULIO_MANUAL = [
    ...
    "AGRICOLA VILLASECA LTDA",  # así en Odoo (no "LIMITADA")
    "COMERCIALIZACIONES  DE MADERAS Y FERRETERIA  DYS SPA",  # doble espacio: nombre EXACTO de la ficha Odoo

    "AGROCONSTRUCCION SPA",
    "TRANSPORTE DE CARGA PRESTACION DE SERVICIO SPA",  # 'SERVICIO' (typo corregido)
    "HUGO HERNAN DONOSO POBLETE",
]

# Cohorte CHURN de agosto-2026 (lista de Madelaine vía Pauline, 21-sep): última
# compra ~nov-2025, cruzaron los 270d en agosto. No salían en Nuevos Perdidos por
# los huecos conocidos (7 sin lead CRM; 6 con lead pegado en Durmiente por la
# regla 3 congelada). Nombres EXACTOS de Odoo (resueltos por RUT).
CHURN_AGOSTO_MANUAL = [
    "MIGUEL ANGEL VALDES MENDOZA",       # compró 08-sep: la graduación lo salta mientras esté recuperado
    "LUIS ROBESPIER MUNOZ VELIZ",
    "SEBASTIAN ANDREE JUILLERAT OLIVA",
    "JULIO CESAR ROJAS CORTES",
    "CARLOS LUIS DURAN ULLOA",
    "JUAN ANTONIO HORMAZABAL MEZA",
    "ERIKA DEL PILAR GUZMAN MANRIQUEZ",
    "JOSE VICTOR CALDERON PARRA",
    "MARIA LETICIA VANNINI ARRIAGADA",   # su factura del 01-sep fue anulada: churn vigente
    "DANIEL ESPINOZA VALLADARES",
    "OLIVOS DE TALCA SPA",
    "GUZMAN Y PENA SERVICIOS LIMITADA",
    "MIGUEL ANGEL DEL C. LECAROS MELO SERVICIOS AGRICOLAS EIRL",
    "SERVITRAC OTEIZA SPA",
    "AGRICOLA Y COMERCIAL PINOR SPA",    # compró 04-sep: graduado mientras dure
    "MADERAS INSIGNE LTDA",
    "LUIS ZENON PALOMINOS CORDOVA",      # compró 08-sep: graduado mientras dure
]
...
def avg_monthly_litros_activos(models, uid, pids, start_date):
    """Prom. mensual de litros diesel (neto de NC) dividido por MESES CON COMPRA
    dentro de la ventana — NO por la ventana completa. Dividir por ventana fija
    diluía a los clientes con pocos meses activos (bug reportado por Pauline
    2026-08-07: Constructora La Esperanza mostraba 368 L/mes = 11.044 L ÷ ventana,
    siendo que sus entregas mínimas son 1.000 L)."""
    res = {}
    pids = [p for p in (pids or []) if p]
    if not pids:
        return res
    month_tot = {}  # (pid, 'YYYY-MM') -> litros netos del mes
    for i in range(0, len(pids), 200):
        ...
        for mt, sign in (("out_invoice", 1), ("out_refund", -1)):
            ...
            ], ["id", "partner_id", "invoice_date"], limit=50000)
            imap = {v["id"]: (safe_id(v.get("partner_id")), (v.get("invoice_date") or "")[:7]) for v in invs}
            ids = list(imap)
            for j in range(0, len(ids), 2000):
                for ln in sr(models, uid, "account.move.line", [
                    ["move_id", "in", ids[j:j+2000]],
                    ["product_id", "=", DIESEL_PRODUCT_ID],
                ], ["move_id", "quantity"], limit=50000):
                    pid, ym = imap.get(safe_id(ln.get("move_id")), (None, None))
                    if pid:
                        month_tot[(pid, ym)] = month_tot.get((pid, ym), 0) + sign * (ln.get("quantity") or 0)
    tot = {}
    act = {}
    for (pid, ym), q in month_tot.items():
        if q > 0:
            tot[pid] = tot.get(pid, 0) + q
            act[pid] = act.get(pid, 0) + 1
    return {pid: round(tot[pid] / act[pid]) for pid in tot if act.get(pid)}
...
def extract_churn_data(models, uid):
    ...
    avg_litros_map = {}
    if dormant_pids:
        avg_litros_map = avg_monthly_litros_activos(models, uid, dormant_pids, fmt(eight_months_start))
    ...
    if lost_pids:
        ...
        avg_litros_lost = avg_monthly_litros_activos(models, uid, lost_pids, fmt(lost_lookback_start))
    ...
    _resc_litros = avg_monthly_litros_activos(models, uid, _resc_pids, fmt(today - timedelta(days=240)))
    for r in _resc_all:
    ...
    _churn_pend = []
    # Cohortes manuales de churn: (lista, etiqueta, fecha_perdido de referencia)
    for _cohorte, _tag, _fecha in [(CHURN_JULIO_MANUAL, "jul-26", "2026-07-31"),
                                   (CHURN_AGOSTO_MANUAL, "ago-26", "2026-08-31")]:
        for _nm in _cohorte:
            _k = _nm.strip().upper()
            if _k in _ll_names:
                _ll_names[_k]["churn_mes"] = _tag
                continue
    ...
    # Manuales completos + top-30 automáticos (antes los manuales comían el cupo
    # de los automáticos; con 2 cohortes fijadas el cap compartido dejaba 0 autos)
    _lost_output = _lost_man + _lost_auto[:30]
...
def extract_recovery_clients(models, uid):
    ...
    RECUPERABLES_MANUAL = CHURN_JULIO_MANUAL + CHURN_AGOSTO_MANUAL
